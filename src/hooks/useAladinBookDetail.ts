...
import { toggleLikeBook } from "@/api/books.api";
...
import { fetchBookDetail } from "@/api/aladin.api";
...
export const useAladinBookDetail = (bookId: string | undefined) => {
  ...
  const { data: bookDetail, isLoading: isBookDetailLoading } = useQuery({
    ...
    queryFn: () => (bookId ? fetchBookDetail(bookId) : Promise.resolve(null)),
  });
  ...
  const { mutate: addToCart } = useMutation({
    mutationFn: async ({ bookId, quantity, info }: addToCartParams) => {
      const { cartItemsCount, message } = await requestAddToCart({ bookId, quantity, info });
  ...
  const { mutate: toggleLike } = useMutation({
    mutationFn: async ({ bookId, info }: LikeRequest) => {
      const { likes, message } = await toggleLikeBook(bookId, info);
